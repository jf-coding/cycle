...
/**
 * The addc instruction class.
 */
public final class addc extends Instruction
{ 
 ...
 /**
  * Instantiates a new addc instruction.
  *
  ...
  */
 public addc(fw_3sp cpu,int address,int rD,int rA,int rB,int latency)
 {
  this.cpu = cpu;
  pc = address;
  this.rD = rD;
 ...
 public final int Stage(int stage)
 {
  ...
  aux_c = aux_a + aux_b + carry;
  cpu.putGeneral(rD,aux_c);
  cpu.setMSRCarry(aux_a,aux_b);
  if(cpu.getMSRCarry() == 0)
  {
   aux_c = aux_a + aux_b;
   cpu.setMSRCarry(aux_c,carry);
  }
