...
/**
 * The lwi instruction class.
 */
public class lwi extends Instruction
{
 ...
 /**
  * Instantiates a new lwi instruction.
  *
  ...
  */
 public lwi(Memory memory,fw_3sp cpu,int address,int rD,int rA,int imm,int latency)
 {
  this.cpu = cpu;
  this.memory = memory;
  pc = address; 
 ...
 public final int Stage(int stage)
 {
  ...
  switch(status)
  {
   ...
   case Mem_Status.UNALIGNED:esr = 0x800 | (rD << 5);
                             cpu.putESR(esr);
	                         cpu.putEAR(address);
	                         cycles = 1;
