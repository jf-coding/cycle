...
public class swi extends Instruction
{
 ...
 /**
  * Instantiates a new swi instruction.
  *
  ...
  */
 public swi(Memory memory,fw_3sp cpu,int address,int rD,int rA,int imm,int latency)
 {
  this.cpu = cpu;
  this.memory = memory;
  pc = address;
 ...
 public final int Stage(int stage)
 {
  ...
  switch(status)
  {
   ...
   case Mem_Status.UNALIGNED:esr = 0xc00 | (rD << 5);
                             cpu.putESR(esr);
	                         cpu.putEAR(address);
	                         cycles = 1;
