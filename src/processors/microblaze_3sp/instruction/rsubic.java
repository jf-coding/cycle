...
/**
 * The rsubic instruction class.
 */
public class rsubic extends Instruction
{
 ...
 /**
  * Instantiates a new rsubic instruction.
  *
  ...
  */
 public rsubic(fw_3sp cpu,int address,int rD,int rA,int imm,int latency)
 {
  this.cpu = cpu;
  pc = address;
  this.rD = rD;
 ...
 public final int Stage(int stage)
 {
  ...
  aux_b = imm + ~aux_a + carry;
  cpu.putGeneral(rD,aux_b);  
  cpu.setMSRCarry(imm,~aux_a);
