...
/**
 * The idivu instruction class.
 */
public class idivu extends Instruction
{
 ...
 /**
  * Instantiates a new idivu instruction.
  *
  ...
  */
 public idivu(fw_3sp cpu,int address,int rD,int rA,int rB,int latency)
 {
  this.cpu = cpu;
  pc = address;
  this.rD = rD;
 ...
 public final int Stage(int stage)
 {
  ...
  if(aux_a == 0)
  {
   ...
   msr = msr | 0x40;
   cpu.putMSR(msr);
   return cpu_status.DBZ;
  }
  else
  {
   aux_b = cpu.getGeneral(rB);
   aux_c = (int)(((long)aux_b & 0x00000000ffffffffL) / ((long)aux_a & 0x00000000ffffffffL));
