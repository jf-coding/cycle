...
/**
 * The addic instruction class.
 */
public final class addic extends Instruction
{
 ...
 /**
  * Instantiates a new addic instruction.
  *
  ...
  */
 public addic(fw_3sp cpu,int address,int rD,int rA,int imm,int latency)
 {
  this.cpu = cpu;
  pc = address;
  this.rD = rD;
 ...
 public final int Stage(int stage)
 {
  ...
  cpu.setMSRCarry(aux_a,imm);
  aux_b = aux_a + imm + carry;
  cpu.putGeneral(rD,aux_b);
  cpu.setMSRCarry(aux_a,imm);
  if(cpu.getMSRCarry() == 0)
  {
   aux_b = aux_a + imm;
   cpu.setMSRCarry(aux_b,carry);
  }
