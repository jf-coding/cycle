...
/**
 * The mts instruction class.
 */
public class mts extends Instruction
{
 ...
 /**
  * Instantiates a new mts instruction.
  *
  ...
  */
 public mts(fw_3sp cpu,int address,int rS,int rA,int latency)
 {
  this.cpu = cpu;
  pc = address; 
  this.rA = rA;
 ...
 public final int Stage(int stage)
 {
  ...
  switch(rS)
  {
   case 0x0001:aux_a = cpu.getGeneral(rA);
               cpu.putMSR(aux_a);
               carry = cpu.getMSRCarry();
               cpu.setMSRCarry(carry);
               return cpu_status.NORMAL;
   case 0x0007:aux_a = cpu.getGeneral(rA);
               cpu.putFSR(aux_a);
               return cpu_status.NORMAL;
  }
  return cpu_status.NORMAL;
