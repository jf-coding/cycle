...
/**
 * The rsubc instruction class.
 */
public class rsubc extends Instruction
{
 ...
 /**
  * Instantiates a new rsubc instruction.
  *
  ...
  */
 public rsubc(fw_3sp cpu,int address,int rD,int rA,int rB,int latency)
 {
  this.cpu = cpu;
  pc = address;  
  this.rD = rD;
 ...
 public final int Stage(int stage)
 {
  ...
  aux_c = aux_b + ~aux_a + carry;
  cpu.putGeneral(rD,aux_c);  
  cpu.setMSRCarry(aux_b,~aux_a);
  if(cpu.getMSRCarry() == 0)
  {
   aux_d = aux_b + ~aux_a;
   cpu.setMSRCarry(aux_d,carry);
  }
